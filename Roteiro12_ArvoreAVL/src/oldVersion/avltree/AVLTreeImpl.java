...
public class AVLTreeImpl<T extends Comparable<T>> 
	...
	//AUXILIARY
	protected void leftRotation(BTNode<T> node, BTNode<T> parent, BTNode<T> grand){
		grand.setRight(parent.getLeft());
		parent.setParent(grand.getParent());
		grand.setParent(parent);
		parent.setLeft(grand);
		
		if (getRoot().equals(parent.getLeft() ))
			root = (BSTNode<T>) parent;
	}
	
	//AUXILIARY
	protected void rightRotation(BTNode<T> node, BTNode<T> parent, BTNode<T> grand){
		grand.setLeft(parent.getRight());
		parent.setParent(grand.getParent());
		grand.setParent(parent);
		parent.setRight(grand);
		
		if (getRoot().equals(parent.getRight()))
			root = (BSTNode<T>) parent;
